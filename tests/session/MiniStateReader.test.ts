...
import {
  ...
  type MiniStateFlags,
  MiniStateReader,
} from "../../src/session/MiniStateReader.js";
  ...
  return {
    client: {
      async read8(address: number): Promise<number> {
        ...
        if (address === map.wIsInBattle) return values.battle ?? 0;
        if (address === map.wTextBoxID) return values.textBoxId ?? 0;
        if (address === map.wLetterPrintingDelayFlags) return values.letterDelay ?? 0;
        if (address === map.wCurMap) return values.mapId ?? 1;
        if (address === map.wPartyCount) return values.partyCount ?? 1;
        if (address === map.wWalkCounter) return values.walkCounter ?? 0;
        if (address === map.wJoyIgnore) return values.joyIgnore ?? 0;
        if (address === map.wNamingScreenType) return values.namingScreenType ?? 0;
        if (address === RWY_ADDRESS) return values.windowY ?? WINDOW_HIDDEN_Y;
        return 0;
      },
      async read16(): Promise<number> {
        return 0;
      },
      async readRange(address: number, length: number): Promise<Uint8Array> {
        readRangeCalls.push({ address, length });
        if (address === map.wYCoord) {
          return Uint8Array.from([values.y ?? 7, values.x ?? 4]);
        }
        return values.tileMap ?? new Uint8Array(length);
      },
    },
    read8Calls,
    readRangeCalls,
...
describe("classifyMiniStateMode", () => {
  ...
  it("prioritizes battle over naming and dialog evidence", () => {
    expect(
      classifyMiniStateMode(
        flags({ battle: 2, namingScreenType: 1, screenText: "lower case", windowY: 120 })
      )
    ).toBe("battle");
  });
  ...
  it("classifies naming screens before ordinary dialog", () => {
    expect(
      classifyMiniStateMode(flags({ namingScreenType: 1, screenText: "lower case", windowY: 120 }))
    ).toBe("naming");
  });
...
describe("createMiniState", () => {
  it("records readiness lock reasons from joyIgnore, walk animation, and dialog", () => {
    const mini = createMiniState(flags({ joyIgnore: 0xff, walkCounter: 2, windowY: 120 }));

    expect(mini.readiness.ready).toBe(false);
    expect(mini.readiness.lockReasons).toEqual(["joy-ignore", "walk-animation", "text-window"]);
  });
});

describe("MiniStateReader", () => {
  it("reads the mini-state RAM addresses and returns a classified state", async () => {
    const reader = createReader({ battle: 1, mapId: 12, y: 3, x: 9, partyCount: 2 });
